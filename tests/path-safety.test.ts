...
import { scanScenes, parseFrontmatter, readProjectSnapshot } from '../extensions/novel-core.ts';
...
test('rejects lexical escape, prefix tricks, NUL and escaping symlink ancestors', () => fixture((root, outside) => {
  ...
  assert.equal(projectPath(root, 'notes/new.md'), path.join(fs.realpathSync(root), 'notes/new.md'));
}));
...
test('scanner preserves actual paths and rejects duplicate addresses', () => fixture(root => {
  ...
  assert.equal(scanScenes(root, 'short-story').get('01-01')?.filePath, fs.realpathSync(path.join(dir, 'scene-01.md')));
...
test('loaded project roots and scene paths share the same canonical base', () => fixture((root, outside) => {
  fs.writeFileSync(path.join(root, 'project.json'), JSON.stringify({ format: 'flash-fiction' }));
  fs.mkdirSync(path.join(root, 'manuscript'));
  fs.writeFileSync(path.join(root, 'manuscript/story.md'), 'A quiet scene.');
  const alias = path.join(outside, 'selected-project');
  fs.symlinkSync(root, alias, 'dir');
  const loaded = readProjectSnapshot(alias);
  assert.equal(loaded.rootPath, fs.realpathSync(root));
  assert.equal(path.relative(loaded.rootPath, loaded.scenes.get('01-01')!.filePath).replaceAll('\\', '/'), 'manuscript/story.md');
}));
